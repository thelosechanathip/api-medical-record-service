...
exports.GenerateForm = async (req, res) => {
    ...
    try {
        ...
        if (ip) {
            ...
            const ifoe = await mraM.InsertFormOpdEr(fiPayload)
            if (ifoe) {
                ...
                const cdPayload = {
                    form_opd_er_id: ifoe.form_opd_er_id,
                    ...cdD,
                    ...fullnamePayload
                }
                const IFoeCdr = await mraM.InsertFormOpdErClinicalDetailResult(cdPayload)
                if (IFoeCdr) {
                    /*
                    ...
                    */
                    ...
                    await mraM.InsertFormOpdErContentOfMedicalRecordResult(setComrPayload)
                }
            }
        }
        ...
        /*
            ...
            อ้างอิงจาก
                patient_id
        */
        const result = await mraM.FetchOneMedicalRecordAuditOpdEr(ip.patient_id)
...
exports.RemoveData = async (req, res) => {
    try {
        ...
        const fpiifi = await mraM.FetchPatientIdInFormOpdEr(fvip.patient_id)
        if (fpiifi && fpiifi != null) {
            // Remove ข้อมูล form_opd_er_review_status_results จำนวนหลาย record อ้างอิงจาก form_opd_er_id
            await mraM.RemoveFormOpdErReviewStatusResult(fpiifi.form_opd_er_id)

            // Remove ข้อมูล form_opd_er_content_of_medical_record_results จำนวนหลาย record อ้างอิงจาก form_opd_er_id
            await mraM.RemoveFormOpdErContentOfMedicalRecordResult(fpiifi.form_opd_er_id)

            // Remove ข้อมูล form_opd_er_content_of_medical_record_results จำนวนหลาย record อ้างอิงจาก form_opd_er_id
            await mraM.RemoveFormClinicalDetailResult(fpiifi.form_opd_er_id)

            await mraM.RemovePdf(fpiifi.form_opd_er_id)

            // Remove ข้อมูล form_opd_ers จำนวน 1 record อ้างอิงจาก form_opd_er_id
            await mraM.RemoveFormOpdEr(fpiifi.form_opd_er_id)
        }
